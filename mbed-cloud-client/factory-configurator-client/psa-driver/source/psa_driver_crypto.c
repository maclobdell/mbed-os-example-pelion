...
static void set_generic_attr_item(uint32_t extra_flags, psa_key_attributes_t *psa_key_attr)
{
...
static void set_se_attr_item(uint32_t extra_flags, psa_key_attributes_t *psa_key_attr)
{
    //Set physical slot number of the SE item
    psa_set_key_slot_number(psa_key_attr,(psa_key_slot_number_t)((extra_flags & PSA_CRYPTO_SLOT_MASK_FLAG) >> PSA_CRYPTO_SLOT_FLAG_OFFSET));

    //Set lifetime of the SE item according to retrieved SE driver lifetime 
...
/**
* Internal API to import/generate/register keys based on an provided attributes and returns it's opened id and handle.
...
*    @param[in] extra_flags extra flags
...
*/
static kcm_status_e assign_key_by_attr( const void* raw_data,
                                       ...
                                       psa_key_handle_t* psa_key_handle)
{
    ...
    *psa_key_handle = PSA_CRYPTO_INVALID_KEY_HANDLE;
    ...
    SA_PV_ERR_RECOVERABLE_RETURN_IF(((extra_flags & PSA_CRYPTO_LOCATION_MASK_FLAG)== PSA_CRYPTO_SECURE_ELEMENT_LOCATION_FLAG), KCM_STATUS_INVALID_PARAMETER, "Invalid item type");
#endif
    ...
    for (size_t i = 0; i < PSA_CRYPTO_NUM_OF_ID_ENTRIES; i++)
    {
        ...
        if (((extra_flags & PSA_CRYPTO_LOCATION_MASK_FLAG) == PSA_CRYPTO_SECURE_ELEMENT_LOCATION_FLAG) &&
            ((extra_flags & PSA_CRYPTO_GENERATION_MASK_FLAG) != PSA_CRYPTO_GENERATION_FLAG)) {
            ...
            SA_PV_ERR_RECOVERABLE_RETURN_IF((psa_status != PSA_SUCCESS && psa_status != PSA_ERROR_ALREADY_EXISTS),
                                            psa_drv_translate_to_kcm_error(psa_status), "Failed to register a SE key (%" PRIi32 ")", (int32_t)psa_status);

        } else
#endif
#endif
        {
            if (raw_data_size == 0 && ((extra_flags & PSA_CRYPTO_GENERATION_MASK_FLAG) == PSA_CRYPTO_GENERATION_FLAG)) {
...
kcm_status_e psa_drv_crypto_assign( const void* data, size_t data_size, uint32_t extra_flags, uint16_t *ksa_id)
{
...
#ifdef MBED_CONF_MBED_CLOUD_CLIENT_SECURE_ELEMENT_ATCA_SUPPORT
    if ((extra_flags & PSA_CRYPTO_LOCATION_MASK_FLAG) == PSA_CRYPTO_SECURE_ELEMENT_LOCATION_FLAG) {
        set_se_attr_item( extra_flags, &psa_key_attr);
    } else
#endif
#endif
    {
    ...
    //Set item attributes
    set_generic_attr_item(extra_flags, &psa_key_attr);

    kcm_status = assign_key_by_attr(raw_key, raw_key_act_size, extra_flags, &psa_key_attr, ksa_id, &psa_key_handle);
    SA_PV_ERR_RECOVERABLE_GOTO_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status = kcm_status, exit, "Failed to import/generate/register new key");
                                                            ...
                                                            psa_key_handle_t* new_pub_psa_key_handle)
{
    ...
    kcm_status = assign_key_by_attr(NULL, 0, extra_flags, &psa_key_attr, new_prv_ksa_id, new_prv_psa_key_handle);
    ...
    if (exist_pub_ksa_id != PSA_INVALID_SLOT_ID) {
        ...
        kcm_status = assign_key_by_attr(raw_pub_key, actual_data_size, 0, &psa_key_attr, new_pub_ksa_id, new_pub_psa_key_handle);
...
kcm_status_e psa_drv_crypto_se_private_key_get_slot(uint16_t psa_prv_key_id, uint16_t *se_prv_key_id)
{
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    psa_key_attributes_t psa_key_attr = PSA_KEY_ATTRIBUTES_INIT;
    psa_key_handle_t exist_psa_key_handle = PSA_CRYPTO_INVALID_KEY_HANDLE;
...
exit:
    ...
    {    // assuming that slot numbers in the driver are not bigger than UINT16_MAX 
        *se_prv_key_id = (uint16_t)slot_number;
        SA_PV_LOG_TRACE_FUNC_EXIT("returning se_prv_key_id = %" PRIu16,  *se_prv_key_id);
    }
    else 
    {
        SA_PV_LOG_TRACE_FUNC_EXIT_NO_ARGS();
