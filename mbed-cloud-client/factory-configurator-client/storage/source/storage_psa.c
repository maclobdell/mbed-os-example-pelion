    ...
    size_t kcm_item_data_size)
{
    ...
    storage_type_location_e item_location = STORAGE_PSA_TYPE_LOCATION;
    ...
    kcm_status = ksa_item_store(storage_item_name, storage_flags, kcm_item_type, kcm_item_data, kcm_item_data_size, item_location, kcm_item_is_factory);
    ...
    bool is_write_once)
{
    ...
    kcm_status = ksa_item_store((const uint8_t *)storage_item_name, storage_flags, STORAGE_RBP_ITEM, data, data_size, STORAGE_PSA_TYPE_LOCATION, false);
...
kcm_status_e storage_cert_chain_add_next_impl(kcm_cert_chain_handle kcm_chain_handle, const uint8_t *kcm_cert_data, size_t kcm_cert_data_size, storage_item_prefix_type_e item_prefix_type)
{
    ...
    kcm_status = ksa_item_store((const uint8_t*)storage_cert_name, storage_flags, KCM_CERTIFICATE_ITEM, kcm_cert_data, kcm_cert_data_size, item_location, chain_context->is_factory);
...
kcm_status_e storage_factory_reset()
{
    ...
    kcm_status = ksa_item_store((const uint8_t*)storage_item_name, 0, STORAGE_RBP_ITEM, NULL, 0, STORAGE_PSA_TYPE_LOCATION, false);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed storing item in PSA (%u)", kcm_status);
    ...
    const kcm_security_desc_s         kcm_item_info)
{
    ...
    size_t pub_key_size;
    storage_type_location_e prv_key_location = STORAGE_PSA_TYPE_LOCATION;
    storage_type_location_e pub_key_location = STORAGE_PSA_TYPE_LOCATION;
    ...
    prv_key_location = (item_extra_info.priv_key_location == KCM_LOCATION_PSA) ? STORAGE_PSA_TYPE_LOCATION : STORAGE_SE_GENERATED_TYPE_LOCATION;
    ...
    if (public_key_name != NULL) {
        ...
        pub_key_location = (item_extra_info.pub_key_location == KCM_LOCATION_PSA) ? STORAGE_PSA_TYPE_LOCATION : STORAGE_SE_GENERATED_TYPE_LOCATION;
    }
#else
