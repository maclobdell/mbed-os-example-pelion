...
typedef struct _ksa_item_entry {
    ...
    uint8_t  item_extra_info;  // Item info - 2 LSB indicate item location:
                                /* 00 - non psa
                                   01 - psa
                                   02 - secure element preprovisioned type location
                                   03 - secure element generated type location
                                */
...
#define KSA_STORAGE_TYPE_LOCATION_MASK      0x3 // use this with item_extra_info to get storage location
...
static kcm_status_e destroy_all_ce_keys()
{
    ...
    while (table_entry <= g_ksa_desc[KSA_KEY_ITEM].ksa_last_occupied_entry) {
        if (table_entry->renewal_item_id != 0) {
            // Destroy the CE key. 
            psa_drv_delete_f delete_data = (psa_drv_delete_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_DELETE, KSA_KEY_ITEM, (storage_type_location_e)(table_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK));
...
static kcm_status_e store_item_to_entry(ksa_item_entry_s *item_entry, ksa_item_type_e ksa_item_type, bool is_new_entry, const uint8_t *item_name, const uint16_t  psa_item_id, bool is_factory, storage_type_location_e item_location)
{
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
...
/* The function set psa flags form storage flags,
  In case the item is not in Secure Element and certificate, configuration item, RRBP item or symmetric key,
  the psa flags re writed by storage flags for ps driver, otherwise the psa flags composed for crypto driver.
*/
static kcm_status_e set_psa_driver_flags(uint32_t item_type, 
                                         storage_type_location_e item_location,
                                         ...
                                         uint32_t *psa_flags)
{
    ...
    switch (item_type) {
        ...
        case KCM_PUBLIC_KEY_ITEM:
            *psa_flags = PSA_CRYPTO_PUBLIC_KEY_FLAG;
            break;
        case KCM_CERTIFICATE_ITEM:
            if (item_location == STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION || 
                item_location == STORAGE_SE_GENERATED_TYPE_LOCATION) {
        ...
        case KCM_SYMMETRIC_KEY_ITEM:
        case STORAGE_RBP_ITEM:
            *psa_flags = storage_flags;//ps flags
            return kcm_status;
        default:
    ...
    if (item_location == STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION || 
        item_location == STORAGE_SE_GENERATED_TYPE_LOCATION) {
        *psa_flags = *psa_flags | PSA_CRYPTO_SECURE_ELEMENT_LOCATION_FLAG;
        *psa_flags = *psa_flags |(uint32_t)(slot_number << PSA_CRYPTO_SLOT_FLAG_OFFSET);
    }
                                                ...
                                                bool is_new_entry)
{
    ...
    psa_drv_store_f store_func = (psa_drv_store_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_WRITE, ksa_item_type, item_location);
    ...
    kcm_status = store_item_to_entry(ksa_item_entry, ksa_item_type, is_new_entry, (uint8_t*)item_name, psa_item_id, kcm_item_is_factory, item_location);
                            ...
                            bool kcm_item_is_factory)
{
    ...
    SA_PV_ERR_RECOVERABLE_RETURN_IF((item_location == STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION), 
                                    KCM_STATUS_INVALID_PARAMETER, 
                                    "Wrong item location type (=%" PRIx8 ")", (uint8_t)item_location);
    ...
    if (item_location == STORAGE_SE_GENERATED_TYPE_LOCATION) {
        if (item_type == KCM_PRIVATE_KEY_ITEM) {
            //Set driver flags
            kcm_status = set_psa_driver_flags(item_type, item_location, need_to_generate, 2, storage_flags, &psa_flags);//Free slot for private key =2
            SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed to set ps flags");
        }
        if (item_type == KCM_PUBLIC_KEY_ITEM) {
            //Set driver flags
            kcm_status = set_psa_driver_flags(item_type, item_location, need_to_generate, 13, storage_flags, &psa_flags);//Free slot for public key =13
            SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed to set ps flags");
        }
    } else {
        //Set driver flags
        kcm_status = set_psa_driver_flags(item_type, item_location, need_to_generate, 0, storage_flags, &psa_flags);
        SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed to set ps flags");
    }
    ...
    kcm_status = store_item_and_update_table(item_name, ksa_item_entry, ksa_item_type, item_location, item_data, item_data_size, psa_flags, kcm_item_is_factory, is_new_entry);
                                    ...
                                    size_t *item_data_size_out)
{
    ...
    psa_drv_get_data_size_f read_size_func = (psa_drv_get_data_size_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_READ_SIZE, ksa_item_type, (storage_type_location_e)(table_item_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK));
                               ...
                               size_t *item_data_act_size_out)
{
    ...
    //Use dispatcher to determinate read data function
    psa_drv_get_data_f read_func = (psa_drv_get_data_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_READ, ksa_item_type, (storage_type_location_e)(table_item_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK));
                             ...
                             uint32_t item_type)
{
    ...
    // verify that the key is not located in pre-provisioned items
    storage_type_location_e item_location = (storage_type_location_e)(ksa_item_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION == item_location), 
                                    KCM_STATUS_INVALID_EXPECTED_LOCATION, 
                                    "item location(=%" PRIx8 ") is in preprovisioned items(=%" PRIx8 "), which can't be deleted", 
                                    (uint8_t)item_location, (uint8_t)STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION);

    // Use dispatcher to determinate "delete" function
    psa_drv_delete_f delete_data = (psa_drv_delete_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_DELETE, ksa_item_type,
        (storage_type_location_e)(ksa_item_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK));
...
kcm_status_e ksa_reset(void)
{
    ...
    */
    ...
    for (uint32_t table_index = KSA_KEY_ITEM; table_index < KSA_LAST_ITEM; table_index++) {
        ...
        for (int entry_index = 0; table_entry <= last_occupied_entry; entry_index++, table_entry++) {
            ...
            if (table_entry->active_item_id != PSA_INVALID_SLOT_ID && table_entry->active_item_id == table_entry->factory_item_id) { //Active and factory with the same value
                delete_data = (psa_drv_delete_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_DELETE, table_index, (storage_type_location_e)(table_entry->item_extra_info & (uint8_t)KSA_STORAGE_TYPE_LOCATION_MASK));
                //Call delete function
    ...
    for (ksa_reserved_id_type_e reserved_data_index = KSA_PS_LAST_FREE_CRYPTO_ID_RESERVED_TYPE; reserved_data_index < KSA_MAX_ID_RESERVED_TYPE; reserved_data_index++) {
        delete_data = (psa_drv_delete_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_DELETE, KSA_CONFIG_ITEM, STORAGE_PSA_TYPE_LOCATION);
        //Call delete function
    ...
    for (ksa_item_type_e table_index = KSA_KEY_ITEM; table_index < KSA_RBP_ITEM; table_index++) {
        ...
        while (table_entry <= g_ksa_desc[table_index].ksa_last_occupied_entry) {
            ...
            psa_drv_delete_f delete_data = (psa_drv_delete_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_DELETE, table_index,
                (storage_type_location_e)(table_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK));
...
kcm_status_e ksa_get_item_location(const uint8_t *item_name, uint32_t item_type, kcm_item_location_e *item_location_out)
{
    ...
    storage_type_location_e ksa_location = (storage_type_location_e)(table_key_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK); 
    switch(ksa_location)
    {
    case STORAGE_PSA_TYPE_LOCATION:
        *item_location_out = KCM_LOCATION_PSA;
        break;
    case STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION: // map both to KCM_LOCATION_SECURE_ELEMENT
    case STORAGE_SE_GENERATED_TYPE_LOCATION:
        *item_location_out = KCM_LOCATION_SECURE_ELEMENT;
        break;

    default: // all other cases are error
    //      case STORAGE_MAX_TYPE_LOCATION:
...
kcm_status_e ksa_se_private_key_get_slot(const uint8_t *prv_key_name, uint16_t *se_prv_key_slot)
{
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    ksa_item_entry_s *table_key_entry = NULL;
    ...
    storage_type_location_e item_location = (storage_type_location_e)(table_key_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((STORAGE_SE_GENERATED_TYPE_LOCATION != item_location && STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION != item_location), 
                                    KCM_STATUS_INVALID_EXPECTED_LOCATION, 
                                    "key location(=%" PRIx8 ") is not SE location(=%" PRIx8 " or %" PRIx8 ")", 
                                    (uint8_t)item_location, (uint8_t)STORAGE_SE_GENERATED_TYPE_LOCATION, (uint8_t)STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION);
...
kcm_status_e ksa_load_item_to_entry_from_se(const uint8_t *item_name, uint32_t item_type, uint16_t slot_number)
{
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    ...
    if (kcm_status == KCM_STATUS_FILE_EXIST) {
        ...
        kcm_item_location_e item_location = (kcm_item_location_e)(ksa_item_entry->item_extra_info & (uint8_t)KSA_STORAGE_TYPE_LOCATION_MASK);
        if (item_location != KCM_LOCATION_SECURE_ELEMENT) {
    ...
    //Set driver flags
    kcm_status = set_psa_driver_flags(item_type, STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION, false, slot_number, 0, &psa_flags);
    SA_PV_TRACE_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed to set ps flags");

    //Store the item and save a relevant table
    kcm_status = store_item_and_update_table(item_name, ksa_item_entry, ksa_item_type, STORAGE_SE_PRE_PROVISIONED_TYPE_LOCATION, NULL, 0, psa_flags, true, is_new_entry);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed to store_item_and_update_table");
...
kcm_status_e ksa_destroy_ce_key(const uint8_t *item_name, uint32_t item_type)
{
    ...
    psa_drv_delete_f delete_data = (psa_drv_delete_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_DELETE, KSA_KEY_ITEM, (storage_type_location_e)(ksa_key_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK));
    //Call delete function
...
kcm_status_e ksa_destroy_old_active_and_remove_backup_entry(const uint8_t *item_name, uint32_t item_type)
{
    ...
    if (table_entry->active_item_id != table_entry->factory_item_id) {
        ...
        // Use dispatcher to determinate "delete" function
        psa_drv_delete_f delete_data = (psa_drv_delete_f)psa_drv_func_dispatch_operation(PSA_DRV_FUNC_DELETE, ksa_item_type, (storage_type_location_e)(table_entry->item_extra_info & KSA_STORAGE_TYPE_LOCATION_MASK));
        //Call delete function
        kcm_status = delete_data(table_entry->active_item_id);
