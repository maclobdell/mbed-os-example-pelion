                                         ...
                                         const arm_uc_buffer_t *buffer)
{
    ...
    if (buffer && buffer->ptr && result.error == ERR_NONE) {
        ...
        uint32_t page_size = arm_uc_flashiap_get_page_size();
        uint32_t hdr_size = 0;//arm_uc_pal_flashiap_round_up_to_page_size(ARM_UC_PAL_HEADER_SIZE);
...
arm_uc_error_t ARM_UC_PAL_FlashIAP_GetActiveDetailsCypress(arm_uc_firmware_details_t *details)
{
    arm_uc_error_t result = { .code = ERR_INVALID_PARAMETER };
    ...
    printf("Current image header major: 0x%02"PRIX64"\n", major_version);
    printf("Current image header minor: 0x%02"PRIX64"\n\n", minor_version);
