...
{
    void *ctx = calloc( 1, sizeof( psa_key_handle_t ) );
    return( ctx );
}

static void pal_se_pk_free_wrap( void *ctx )
{
    free( ctx );
}

static int pal_se_pk_can_do( mbedtls_pk_type_t type )
{
    return( type == MBEDTLS_PK_ECDSA );
}

int pal_se_pk_sign_wrap( void *ctx, mbedtls_md_type_t md_alg,
                   ...
                   int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    int32_t platStatus = CRYPTO_PLAT_SUCCESS;
    psa_status_t status;
    psa_key_handle_t *key_handle = (psa_key_handle_t*)ctx;
    const uint8_t rawSignature[PAL_ECDSA_SECP256R1_SIGNATURE_RAW_SIZE] = { 0 };
    size_t rawSignatureSize = sizeof(rawSignature);

    status = fb_keys_open_key(FB_KEY_SLOT_DEVICE_ECDSA, key_handle);
    if (status != PSA_SUCCESS) {
        printf("[ERR] fb_keys_open_key FAILED (%d)\r\n", status);
        return CRYPTO_PLAT_GENERIC_ERROR;
    }

    status = fb_psa_asymmetric_sign(*key_handle, PSA_ALG_ECDSA(PSA_ALG_SHA_256),
                                    hash, hash_len, rawSignature, rawSignatureSize, &rawSignatureSize);
    if (status != PSA_SUCCESS) {
        printf("[ERR] fb_psa_asymmetric_sign FAILED (%d)\r\n", status);
        return CRYPTO_PLAT_GENERIC_ERROR;
    }

    status = fb_keys_close_key(FB_KEY_SLOT_DEVICE_ECDSA);
    if (status != PSA_SUCCESS) {
        printf("[ERR] fb_keys_close_key FAILED (%d)\r\n", status);
        return CRYPTO_PLAT_GENERIC_ERROR;
    }

    // Convert raw signature to der format
    platStatus = pal_plat_convertRawSignatureToDer(rawSignature, rawSignatureSize, sig, PAL_ECDSA_SECP256R1_SIGNATURE_DER_SIZE, sig_len);
    if (platStatus != CRYPTO_PLAT_SUCCESS) {
        return CRYPTO_PLAT_GENERIC_ERROR;
    }
...
static int pal_se_pk_verify_wrap( void *ctx, mbedtls_md_type_t md_alg,
                       const unsigned char *hash, size_t hash_len,
...
static int pal_se_pk_check_pair( const void *pub, const void *prv )
{
    return CRYPTO_PLAT_SUCCESS;
}

const mbedtls_pk_info_t pal_se_pk_info = {
    MBEDTLS_PK_OPAQUE,
    "PAL SE",
    NULL,
    pal_se_pk_can_do,
    pal_se_pk_verify_wrap,
    pal_se_pk_sign_wrap,
    ...
    pal_se_pk_check_pair,
    pal_se_pk_alloc_wrap,
    pal_se_pk_free_wrap,
#if defined(MBEDTLS_ECDSA_C) && defined(MBEDTLS_ECP_RESTARTABLE)
...
#endif // TARGET_CY8CKIT_064S2_4343W && !defined(DISABLE_CY_FACTORY_FLOW)
