...
palStatus_t pal_SSTGet(const char *itemName, void *itemBuffer, size_t itemBufferSize, size_t *actualItemSize)
{
    ...
    if (strstr(itemName, g_fcc_bootstrap_device_private_key_name)) {
        if (itemBufferSize < name_len) {
            return PAL_ERR_SST_INVALID_SIZE;
        }
        strncpy(itemBuffer, g_fcc_bootstrap_device_private_key_name, name_len);
        *actualItemSize = name_len;
        return PAL_SUCCESS;
    }
#endif
...
palStatus_t pal_SSTGetInfo(const char *itemName, palSSTItemInfo_t *palItemInfo)
{
    ...
    if (strstr(itemName, g_fcc_bootstrap_device_private_key_name)) {
        ...
        palItemInfo->itemSize = strlen(g_fcc_bootstrap_device_private_key_name);
        return PAL_SUCCESS;
    }
#endif
