...
uint8_t get_led_blink_rate(void);
void set_led_blink_rate(uint8_t);
...
******************************************************************************
* Summary:
*   Main function that starts a thread for CapSense scan 
*
*****************************************************************************/
int capsense_main(void)
{
	...
	printf("initialize capsense interrupt\r\n");
...
void ProcessTouchStatus(void)
{
    ...
    {       
        ...
        if(currSliderPos != prevSliderPos)
        {
			...
			//Set blink Rate
			blinker_rate_set(currSliderPos * 7 / 100);  //scale to 0 - 7
