...
/* Certificate index in the chain_of_trust policy array */
#define DEVICE_CERT_INDEX      0
...
//device  model number
static const char MBED_CLOUD_MODEL_NUMBER[] = "CY8CKIT-064S2-4343W";
//device type
static const char MBED_CLOUD_DEVICE_TYPE[] = "PSOC64";
//device hw version
static const char MBED_CLOUD_HARDWARE_VERSION[] = "Rev 02";
//device total memory
...
static int read_device_cert(uint8_t *derCert, size_t derCertMaxSize, size_t *derCertSizeOut)
{
    ...
    *derCertSizeOut = 0;
    ...
    /* Get provisioning packet from Secure Flash Boot */
    rc = Cy_JWT_GetProvisioningDetails(FB_POLICY_JWT, &jwt, &jwtLen);
    if(0 != rc) {
        printf("[ERR] Cy_JWT_GetProvisioningDetails failed with code %i", rc);
        goto exit;
    }
